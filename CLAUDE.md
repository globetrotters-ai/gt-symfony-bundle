...
| `kernel.response` | `Serving\SitemapFilter` | -20 | Merges the discovery URLs into an app-served `/sitemap.xml`, or generates one on a 404 |
...
- **The sitemap decorates; the artefacts pre-empt.** `Router` runs before routing precisely so nothing can claim the artefact paths, and no app serves `/llms.txt`. Plenty serve `/sitemap.xml`, and that one is theirs — so `SitemapFilter` uses `RobotsFilter`'s decorate-or-generate shape instead, merging the discovery `<url>` entries into the app's own document. **Merging, not standing aside, is load-bearing:** `apex/services/probes/robots_sitemap.py::probe_sitemap_includes_llms` fetches a *hardcoded* `{apex}/sitemap.xml` and fails unless a `<loc>` names `llms.txt`/`llms-full.txt` — it never reads the `Sitemap:` line, so a sitemap at a path of our own would be invisible to it, and standing aside would make the common case (an apex that already has a sitemap) a no-op. The backend tells hand-merging integrators the same thing (`deploy_config_renderer`, the `merge-manually/sitemap.xml` copy). Decoration is additive and refuses anything it cannot merge safely: a `<sitemapindex>`, a compressed/streamed body, one over 1 MB, or one already carrying `Sitemap::MARKER`.
