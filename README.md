...
- **Changing or clearing `website_url` stops serving the previous source at once.** The cached bundle records the URL it was pulled from and is served only while that URL is configured. The next `gt:refresh` drops it and pulls the new source straight away, whatever the interval. On the Scheduler lane, run `gt:refresh --force` after the deploy, or the new source waits for the next scheduled refresh.
